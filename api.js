export default class APIHandler {
    ...
    // TODO: (update) ID로 카드 검색 후 내용,카테고리 수정
    async putCard(cardObj) {    
        this.dummyData = this.dummyData.map(card=>{
            return card.id===cardObj.id ? {...card, category:cardObj.category,
                                                    title:cardObj.title} : card
        });
        console.log(this.dummyData);
    }
    ...
    // TODO: ID로 카드 검색 후 삭제
    async deleteCard(id) {
        // filter : 해당 조건을 만족하는 경우에만 진행.
        this.dummyData = this.dummyData.filter(card=>{ // 파라미터가 card 하나이므로 괄호 생략 (card)=>
            return card.id!==id // 카드의 id가 일치하는지 여부 반환 (true/false)
        });
        console.log(this.dummyData);
    }
